...
def _sse_response(generator):
    """Wrap a text generator into an SSE StreamingHttpResponse."""
    def sse_format():
        for chunk in generator:
            yield f"data: {json.dumps({'text': chunk}, ensure_ascii=False)}\n\n"
        yield "data: [DONE]\n\n"

    response = StreamingHttpResponse(sse_format(), content_type='text/event-stream')
...
@csrf_exempt
def analyze_error_stream(request):
    """流式版本的错误分析"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
...
3. **修复建议**：给出具体的修改方向和示例思路"""
    ...
    return _sse_response(ask_ai_stream(prompt))
...
@csrf_exempt
def problem_hint_stream(request):
    """流式版本的解题提示"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
...
@csrf_exempt
def learning_advice_stream(request):
    """流式版本的学习建议"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        try:
            result = neo4j_client.run_query(query, {'username': username})
            for r in result:
                weak_topics.append({'topic': r['topic'], 'error_rate': round(r['error_rate'] * 100, 1), 'total': r['total_sub']})
        except Exception:
            pass
...
@csrf_exempt
def code_review_stream(request):
    """流式版本的代码审查"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
...
def visualize_code(request):
    ...
    tracer_path = os.path.join(os.path.dirname(__file__), 'code_tracer.py')
    try:
        ...
        if result.returncode == 0:
            ...
            return JsonResponse({'steps': trace_data})
        else:
            return JsonResponse({
                'error': '代码执行失败',
        ...
        return JsonResponse({'error': '执行超时（10秒限制）'}, status=504)
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)


@csrf_exempt
def code_review_structured(request):
    """AI 代码审查：返回结构化 JSON 报告"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    prompt = f"""你是一个专业的代码审查专家。请审查以下代码并以严格的 JSON 格式返回结果，不要包含任何其他文字。
...
请返回以下 JSON 格式（确保是合法 JSON）：
{{
  "overall_score": 0-100的整数,
  "dimensions": {{
    "efficiency": 0-100的整数,
    "readability": 0-100的整数,
    "security": 0-100的整数,
    "style": 0-100的整数
  }},
  "time_complexity": "如 O(n log n)",
  "space_complexity": "如 O(1)",
  ...
  "summary": "总体评价"
    ...
    try:
        ...
        if json_match:
            ...
            return JsonResponse({'review': answer, 'structured': False})
    except json.JSONDecodeError:
        return JsonResponse({'review': answer, 'structured': False})
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
